...
export const useLayoutDashboardStore = defineStore('[LayoutDashboardStore]', () => {
  ...
  const classeFiltro = ref<CClasseFiltro<CPedidoCompraPendenteModel>>(new CClasseFiltro());
  ...
  const filtrarComprasPendentes = ref<(...args: any) => Promise<IPedidoCompraPendenteResponse>>(
    async (...args: any) => {
      const comprasPendentes = await controller.listarComprasPendentes(args[0] || classeFiltro.value);
      return comprasPendentes;
    },
  );

  const pontoDeCompraController = new PontoDeCompraController();
  const filtrarPontoDeCompra = ref<(...args: any) => Promise<IPontoDeCompraResponse>>(async (...args: any) => {
    const pontoDeCompra = await pontoDeCompraController.listarPontoDeCompra(args[0] || classeFiltro.value);
