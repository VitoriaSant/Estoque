      ...
      db.query(countQuery, params, (err: any, countResult: any) => {
        ...
        db.query(queryPaginada, [limite, skip, ...params], (err: any, result: any) => {
          ...
          const dadosMapeados = Array.isArray(result)
            ? result.map((row) => CPedidoCompraPendenteModel.mapearParaModel(row))
            : result
              ? CPedidoCompraPendenteModel.mapearParaModel(result)
              : [];
          ...
          const response = new CResponseConsultaPaginada<CPedidoCompraPendenteModel>({
            paginacao: {
              pagina,
              limite,
              totalDeRegistros,
